# Legacy: self-host deployment (Proxmox LXC / Docker)

> **Not the current flow.** The tracker is hosted at its public site now
> (see [DEPLOYMENT.md](DEPLOYMENT.md)), and local installs are not offered
> or supported. This file preserves the old self-host deployment story —
> the Proxmox Helper-Script containers, the `ptcg-update` deploy loop, the
> published Docker images — for the maintainer's own use and for whenever
> local installs return. The scripts it references (`ct/`, `install/`,
> `misc/`, `Dockerfile`, `docker-compose.yml`) are all still in the repo.

## The old flow at a glance
...
## Operations cheat-sheet (LXC installs)
