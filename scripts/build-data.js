  ...
  for (let si = 0; si < sets.length; si++) {
    ...
    if (!NO_IMAGES) {
      ...
      for (const c of setData.cards) {
        ...
        // — detected for ANY variant key, incl. custom printings (cracked-ice-holo…)
        const vimgs = {};
        let entries = [];
        try { entries = fs.readdirSync(dir); } catch { /* no image dir yet */ }
        for (const f of entries) {
          const m = f.match(/^([a-zA-Z0-9_-]+)-(low|high)\.webp$/);
          if (!m) continue;
          (vimgs[m[1]] = vimgs[m[1]] || []).push(m[2]);
        }
        for (const k of Object.keys(vimgs)) vimgs[k].sort(); // deterministic order
